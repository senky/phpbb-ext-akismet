...
class listener_test extends \phpbb_test_case
{
	...
	protected function get_listener($user, \phpbb\config\config $config = null, \phpbb\log\log_interface $log = null)
	{
		...
		return $this->getMockBuilder(\phpbb\akismet\event\main_listener::class)
			...
			->setMethods(array('group_user_add'))
			->getMock();
	}
	...
	public function test_post_check($username, $message, $mode, $should_pass)
	{
		...
		$listener = $this->get_listener(new \phpbb\akismet\tests\mock\user($username), null, $log);
		$akismet_mock = new \phpbb\akismet\tests\mock\akismet_mock();
		$this->container->set('phpbb.akismet.client', $akismet_mock);
	...
	public function test_registration_check($config, $username, $blatant, $should_pass, $should_add_to_spammy_group, $should_add_to_blatantly_spammy_group)
	{
		...
		$listener = $this->get_listener(new \phpbb\akismet\tests\mock\user($username), new \phpbb\config\config($config), $log);
		...
		$akismet_mock = new \phpbb\akismet\tests\mock\akismet_mock($blatant);
		$this->container->set('phpbb.akismet.client', $akismet_mock);
		$data = array(
			'user_id' => 123,
			'user_row' => array(
	...
	{
		$function_map = \phpbb\akismet\event\main_listener::getSubscribedEvents();
		$this->assertGreaterThan(0, count($function_map), 'No events subscribed');
		$reflection = new \ReflectionClass(\phpbb\akismet\event\main_listener::class);
	...
	{
		$listener = $this->get_listener(new \phpbb\akismet\tests\mock\user('dummy'));
	...
	public function test_normal_spammy_group_deleted()
	{
		...
		$config = new \phpbb\config\config(array(
			'phpbb_akismet_add_registering_spammers_to_group' => 888,
			'phpbb_akismet_add_registering_blatant_spammers_to_group' => 999, // Should remain unchanged
		));
		$listener = $this->get_listener(new \phpbb\akismet\tests\mock\user('dummy'), $config, $log);
		$event = new \phpbb\event\data();
		$event['group_id'] = 123;
		$listener->group_deleted($event);
		$this->assertEquals(888, $config['phpbb_akismet_add_registering_spammers_to_group']);
		$event['group_id'] = 888;
		$listener->group_deleted($event);
		$this->assertEquals(0, $config['phpbb_akismet_add_registering_spammers_to_group']);

		// The other group should remain unchanged by all this
		$this->assertEquals(999, $config['phpbb_akismet_add_registering_blatant_spammers_to_group']);
	}
	...
	public function test_blatantly_spammy_group_deleted()
	{
		...
		$event['group_id'] = 999;
		$listener->group_deleted($event);
		$this->assertEquals(0, $config['phpbb_akismet_add_registering_blatant_spammers_to_group']);
